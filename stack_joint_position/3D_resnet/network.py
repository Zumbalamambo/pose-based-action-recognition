...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, shortcut_type='B', num_classes=101):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(3, 3, 3), stride=2, padding=1) # kernal size from 3*3*3 to 1*3*3, padding from 1 to (0,1,1)
...
def weight_trainsform(pretrain_dict, model_dict):
...
def resnet18(**kwargs):
    ...
    weight_dict = weight_trainsform(pretrain_dict,model_dict)
