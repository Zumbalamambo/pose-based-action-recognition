...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, nb_classes=12):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1_custom = nn.Conv2d(15, 64, kernel_size=7, stride=2, padding=3,   
                               bias=False)
...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
       ...
       model_dict=weight_transform(model_dict, pretrain_dict,15)
...
def resnet50(pretrained=False, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], channel=channel, **kwargs)
    if pretrained:
